...
// Detecta telas touch e usuários que preferem menos movimento.
const isTouch = window.matchMedia('(pointer: coarse)').matches;
const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

// Define quando o parallax deve ficar ativo.
// Em tablets/mobile e simuladores menores, ele é desligado para evitar desalinhamentos.
function canUseParallax() {
  return !isTouch && !reduceMotion && window.innerWidth > 1080;
}
...
// Aplica movimento parallax apenas quando há espaço suficiente para ele.
function animateParallax() {
  ...
  parallaxItems.forEach((item) => {
    ...
    item.style.transform = `translate3d(0, ${offset * -0.25}px, 0)`;
  });
...
// Recalcula estados quando o simulador/responsividade muda de tamanho.
